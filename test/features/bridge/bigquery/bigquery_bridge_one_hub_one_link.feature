Feature: [BQ-BRG-1L] Bridge table
  Base Bridge behaviour with one hub and one link

  @fixture.bridge
  Scenario: [BQ-BRG-1L-001] Base load into a bridge table from one hub and one link with the AS_OF date and LDTS equal
  ...
  @fixture.bridge
  Scenario: [BQ-BRG-1L-003] Base load into a bridge table from one hub and one link with AS_OF dates in the future
    Given the BRIDGE_CUSTOMER_ORDER table does not exist
    And the raw vault contains empty tables
      | HUB          | LINK                | EFF_SAT                | BRIDGE                |
      | HUB_CUSTOMER | LINK_CUSTOMER_ORDER | EFF_SAT_CUSTOMER_ORDER | BRIDGE_CUSTOMER_ORDER |
  ...
  @fixture.bridge
  Scenario: [BQ-BRG-1L-004] Base load into a bridge table from one hub and one link with multiple loads and an encompassing range of AS OF dates
    Given the BRIDGE_CUSTOMER_ORDER table does not exist
    And the raw vault contains empty tables
      | HUB          | LINK                | EFF_SAT                | BRIDGE                |
      | HUB_CUSTOMER | LINK_CUSTOMER_ORDER | EFF_SAT_CUSTOMER_ORDER | BRIDGE_CUSTOMER_ORDER |
