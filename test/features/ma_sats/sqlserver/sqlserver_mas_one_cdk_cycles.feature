Feature: Multi Active Satellites
  Loading in cycles using separate manual loads of MAS behaviour with one CDK

  @fixture.multi_active_satellite_cycle_sqlserver
  Scenario: [SAT-CYCLE] Load over several cycles with sets of records keeping the group size the same while having one or more records changed
  ...
  @fixture.multi_active_satellite_cycle_sqlserver
  Scenario: [SAT-CYCLE] Load over several cycles with a mix of record change cases
    Given the RAW_STAGE stage is empty
    And the MULTI_ACTIVE_SATELLITE ma_sat is empty
