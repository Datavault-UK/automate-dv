Feature: [SF-SEF-AUB] Effectivity Satellites
  ...
  Scenario: [SF-SEF-AUB-01] One load; going from an empty table to 1 CUSTOMER per ORDER
    ...
    Then the EFF_SAT_ORDER_CUSTOMER table should contain expected data
      ...
      | md5('1002\|\|200') | md5('1002') | md5('200') | TRUE   | 2018-06-01 00:00:00.000 | 2018-06-01 00:00:00.000 | *      |
      | md5('1003\|\|300') | md5('1003') | md5('300') | TRUE   | 2018-06-01 00:00:00.000 | 2018-06-01 00:00:00.000 | *      |

  @fixture.enable_auto_end_date
  @fixture.eff_satellite_status_testing_auto_end_dating
  Scenario: [SF-SEF-AUB-02] One load; going from an empty table to the same CUSTOMER for 3 different ORDERS
    Given the EFF_SAT_ORDER_CUSTOMER table does not exist
    And the RAW_STAGE_ORDER_CUSTOMER table contains data
      | CUSTOMER_ID | ORDER_ID | EFFECTIVE_FROM          | LOAD_DATETIME           | SOURCE |
      | 1001        | 100      | 2018-06-01 00:00:00.000 | 2018-06-01 00:00:00.000 | *      |
    ...
    Then the EFF_SAT_ORDER_CUSTOMER table should contain expected data
      | ORDER_CUSTOMER_PK  | CUSTOMER_PK | ORDER_PK   | STATUS | EFFECTIVE_FROM          | LOAD_DATETIME           | SOURCE |
      | md5('1001\|\|100') | md5('1001') | md5('100') | TRUE   | 2018-06-01 00:00:00.000 | 2018-06-01 00:00:00.000 | *      |
      | md5('1001\|\|101') | md5('1001') | md5('101') | TRUE   | 2018-06-01 00:00:00.000 | 2018-06-01 00:00:00.000 | *      |
      | md5('1001\|\|102') | md5('1001') | md5('102') | TRUE   | 2018-06-01 00:00:00.000 | 2018-06-01 00:00:00.000 | *      |

  @fixture.enable_auto_end_date
  @fixture.eff_satellite_status_testing_auto_end_dating
  Scenario: [SF-SEF-AUB-03] One load; going from an empty table to 3 CUSTOMERS per ORDER
    Given the EFF_SAT_ORDER_CUSTOMER table does not exist
    And the RAW_STAGE_ORDER_CUSTOMER table contains data
      | CUSTOMER_ID | ORDER_ID | EFFECTIVE_FROM          | LOAD_DATETIME           | SOURCE |
      | 1001        | 100      | 2018-06-01 00:00:00.000 | 2018-06-01 00:00:00.000 | *      |
    ...
    Then the EFF_SAT_ORDER_CUSTOMER table should contain expected data
      | ORDER_CUSTOMER_PK  | CUSTOMER_PK | ORDER_PK   | STATUS | EFFECTIVE_FROM          | LOAD_DATETIME           | SOURCE |
      | md5('1001\|\|100') | md5('1001') | md5('100') | TRUE   | 2018-06-01 00:00:00.000 | 2018-06-01 00:00:00.000 | *      |
      | md5('1002\|\|100') | md5('1002') | md5('100') | TRUE   | 2018-06-01 00:00:00.000 | 2018-06-01 00:00:00.000 | *      |
      | md5('1003\|\|100') | md5('1003') | md5('100') | TRUE   | 2018-06-01 00:00:00.000 | 2018-06-01 00:00:00.000 | *      |

  # CUSTOMER_FK is DRIVING KEY
  @fixture.enable_auto_end_date
  @fixture.eff_satellite_status_testing_auto_end_dating
  Scenario: [SF-SEF-AUB-04] One load; going from an empty table to 3 ORDERS
