...
{%- macro default__derive_columns(
    ...
    {%- set src_columns = [] -%}
    {%- set der_columns = [] -%}
    {%- set source_cols = dbtvault.source_columns(
        source_relation = source_relation
    ) -%}
            ...
            {%- else -%}
                {%- if column_config is mapping and column_config -%}
                    ...
                    {%- if dbtvault.is_list(
                        ...
                        {%- do der_columns.append(concat_string) -%}
                    {%- else -%}
                        {%- set column_str = dbtvault.as_constant(
                            column_config ['source_column']
                        ) -%}
                        {%- if column_escape is true -%}
                            {%- do der_columns.append(dbtvault.escape_column_names(column_str) ~ " AS " ~ dbtvault.escape_column_names(derived_column)) -%}
                        {%- else -%}
                            {%- do der_columns.append(
                                column_str ~ " AS " ~ dbtvault.escape_column_names(derived_column)
                            ) -%}
                        {%- endif -%}
                    {%- endif -%}
                {%- else -%}
    ...
    {%- else -%}
        {%- if execute -%}
            {{ exceptions.raise_compiler_error(
                "Invalid column configuration: expected format, either: {'source_relation': Relation, 'columns': {column_name: column_value}} or: {'source_relation': Relation, 'columns': {column_name: {'source_column': column_value, 'escape': true / false}}} got: {'source_relation': " ~ source_relation ~ ", 'columns': " ~ columns ~ "}"
            ) }}
        {%- endif %}
    {%- endif %}
{%- endmacro -%}
