...
source_data_with_counts AS (
    ...
    (
        ...
        FROM (SELECT DISTINCT {{ dbtvault.prefix([src_pk], 's') }}, {{ dbtvault.prefix([src_hashdiff], 's') }}, {{ dbtvault.prefix(cdk_cols, 's') }} FROM source_data AS s) AS t
...
matching_records AS (
    ...
    (
        ...
        FROM (SELECT DISTINCT {{ dbtvault.prefix([src_pk], 's') }}, {{ dbtvault.prefix([src_hashdiff], 's') }}, {{ dbtvault.prefix(cdk_cols, 's') }} FROM source_data AS s) AS t
        WHERE {{ dbtvault.prefix([src_pk], 't') }} = {{ dbtvault.prefix([src_pk], 'stage') }}
        GROUP BY {{ dbtvault.prefix([src_pk], 't') }}
    ) ca
    GROUP BY {{ dbtvault.prefix([src_pk], 'stage') }}
),
