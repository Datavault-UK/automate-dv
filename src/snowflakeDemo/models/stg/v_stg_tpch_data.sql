CREATE VIEW DV_PROTOTYPE_DB.STG.v_stg_customer AS
SELECT 
	...
	MD5_BINARY(UPPER(TRIM(CAST(a.LINENUMEBR AS VARCHAR)))) AS LINEITEM_PK,
	MD5_BINARY(CONCAT(
	...
	MD5_BINARY(UPPER(TRIM(CAST(a.CUSTOMER_NATIONKEY AS VARCHAR)))) AS NATION_PK,
	MD5_BINARY(CONCAT(
	...
	MD5_BINARY(UPPER(TRIM(CAST(a.CUSTOMER_REGIONKEY AS VARCHAR)))) AS REGION_PK,
	MD5_BINARY(CONCAT(
		IFNULL(UPPER(TRIM(CAST(a.CUSTOMER_NATIONKEY AS VARCHAR))), '^^'), '||',
		IFNULL(UPPER(TRIM(CAST(a.CUSTOMER_REGIONKEYKEY AS VARCHAR))), '^^'))) AS NATION_REGION_PK,
	MD5_BINARY(CONCAT(
	...
	MD5_BINARY(CONCAT(
		IFNULL(UPPER(TRIM(CAST(a.LINEITEM AS VARCHAR))), '^^'), '||',
	...
	a.*,
	CAST('1993-01-02' AS DATE) AS LOADDATE,
	'TPCH' AS SOURCE
FROM DV_PROTOTYPE_DB.SRC.V_DAY1 AS a;
/* Need to create a hash link between part supply and parts*/
