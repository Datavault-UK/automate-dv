...
def compare_ct_to_db_table_customer(context, table_path, hub=None):
    """
    ...
    if hub:
        ...
        result_df.sort_values(by=["CUSTOMERKEY"], inplace=True)
...
def compare_ct_to_db_table_part(context, table_path, hub=None):
    """
    Compares the context table to the table that exists on the database.
    """
    table_df = context.testdata.context_table_to_df(context.table)
    ...
    if hub:
        select = "SELECT CAST(PART_PK AS VARCHAR(32)) AS PART_PK, PARTKEY, LOADDATE, SOURCE"
        result_df = DataFrame(context.testdata.get_hub_table_data(select, table_path), dtype=str)

        table_df.columns = map(str.upper, table_df.columns)
        result_df.columns = map(str.upper, result_df.columns)
        table_df['PART_PK'] = table_df['PART_PK'].str.upper()
        result_df.sort_values(by=["PARTKEY"], inplace=True)
        result_df.reset_index(inplace=True, drop=True)
