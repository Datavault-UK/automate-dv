Feature: [BQ-SAT-PM-B] Satellites Loaded using Period Materialization for base loads
  ...
  @fixture.satellite
  Scenario: [BQ-SAT-PM-B-01] Base load of a satellite with one value in rank column loads first rank
  ...
  @fixture.satellite
  Scenario: [BQ-SAT-PM-B-02] Incremental load of a satellite with one value in rank column loads all records
    Given the SATELLITE table does not exist
    And the RAW_STAGE table contains data
      | CUSTOMER_ID | CUSTOMER_NAME | CUSTOMER_DOB | CUSTOMER_PHONE  | LOAD_DATE  | SOURCE |
      | 1001        | Alice         | 1997-04-24   | 17-214-233-1214 | 1993-01-01 | *      |
  ...
  @fixture.satellite
  Scenario: [BQ-SAT-PM-B-03] Incremental load of a satellite should not load PK NULLs
    Given the SATELLITE table does not exist
    And the RAW_STAGE table contains data
      | CUSTOMER_ID | CUSTOMER_NAME | CUSTOMER_DOB | CUSTOMER_PHONE  | LOAD_DATE  | SOURCE |
      | 1001        | Alice         | 1997-04-24   | 17-214-233-1214 | 1993-01-01 | *      |
  ...
  Scenario: [BQ-SAT-PM-B-04] Base load of a satellite using full refresh and start and end dates should only contain first period records
  ...
  @fixture.satellite_cycle
  Scenario: [BQ-SAT-PM-B-05] Base load of a satellite should not load PK NULLs
    Given the RAW_STAGE stage is empty
    And the SATELLITE sat is already populated with data
      | CUSTOMER_PK | HASHDIFF                             | CUSTOMER_NAME | CUSTOMER_DOB | EFFECTIVE_FROM | LOAD_DATE  | SOURCE |
      | md5('1001') | md5('1990-02-03\|\|1001\|\|ALBERT')  | Albert        | 1990-02-03   | 2019-05-04     | 2019-05-04 | *      |
  ...
  @fixture.enable_full_refresh
  @fixture.satellite_cycle
  Scenario: [BQ-SAT-PM-B-06] Base load of a satellite using full refresh and only start date should only contain first period records
    Given the RAW_STAGE stage is empty
    And the SATELLITE sat is already populated with data
      | CUSTOMER_PK | HASHDIFF                             | CUSTOMER_NAME | CUSTOMER_DOB | EFFECTIVE_FROM | LOAD_DATE  | SOURCE |
      | md5('1001') | md5('1990-02-03\|\|1001\|\|ALBERT')  | Albert        | 1990-02-03   | 2019-05-04     | 2019-05-04 | *      |
