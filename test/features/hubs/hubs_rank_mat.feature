Feature: [HUB-RM] Hubs Loaded using Rank Materialization
  ...
  Scenario: [HUB-RM-001] Simple load of stage data into an empty hub
  ...
  @fixture.single_source_hub
  Scenario: [HUB-RM-002] Full refresh of loaded hub
    Given the HUB table does not exist
    And the RAW_STAGE table contains data
  ...
  Scenario: [HUB-RM-003] Simple load of stage data from multiple sources into an empty hub
    Given the HUB table does not exist
