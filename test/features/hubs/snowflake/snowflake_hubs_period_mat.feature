Feature: [SF-HUB-PM] Hubs Loaded using Period Materialization
  ...
  Scenario: [SF-HUB-PM-01] Simple load of stage data into an empty hub
  ...
  @fixture.single_source_hub
  Scenario: [SF-HUB-PM-02] Full refresh of loaded hub
    Given the HUB table does not exist
    And the RAW_STAGE table contains data
  ...
  Scenario: [SF-HUB-PM-03] Simple load of stage data from multiple sources into an empty hub
    Given the HUB table does not exist
