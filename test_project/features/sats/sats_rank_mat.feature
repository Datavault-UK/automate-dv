...
Feature: Satellites Loaded using Rank Materialization
  ...
  @fixture.satellite
  Scenario: [SAT-RANK-MAT] Base load of a satellite with one value in rank column loads first rank
    Given the SATELLITE table does not exist
    And the RAW_STAGE table contains data
      | CUSTOMER_ID | CUSTOMER_NAME | CUSTOMER_DOB | CUSTOMER_PHONE  | LOAD_DATE  | SOURCE |
      | 1001        | Alice         | 1997-04-24   | 17-214-233-1214 | 1993-01-01 | *      |
      ...
      | 1003        | Chad          | 2013-02-04   | 17-214-233-1216 | 1993-01-01 | *      |
      | 1004        | Dom           | 2018-04-13   | 17-214-233-1217 | 1993-01-01 | *      |
    And I have a rank column DBTVAULT_RANK in the STG_CUSTOMER_WITH_RANK_TS stage partitioned by CUSTOMER_ID and ordered by CUSTOMER_ID
    And I create the STG_CUSTOMER_WITH_RANK stage
  ...
  @fixture.satellite
  Scenario: [SAT-RANK-MAT] Incremental load of a satellite with one value in rank column loads all records
    Given the SATELLITE table does not exist
    And the RAW_STAGE table contains data
      | CUSTOMER_ID | CUSTOMER_NAME | CUSTOMER_DOB | CUSTOMER_PHONE  | LOAD_DATE  | SOURCE |
      | 1001        | Alice         | 1997-04-24   | 17-214-233-1214 | 1993-01-01 | *      |
      ...
      | 1003        | Chad          | 2013-02-04   | 17-214-233-1216 | 1993-01-01 | *      |
      | 1004        | Dom           | 2018-04-13   | 17-214-233-1217 | 1993-01-01 | *      |
    And I have a rank column DBTVAULT_RANK in the STG_CUSTOMER_WITH_RANK_TS stage partitioned by LOAD_DATE and ordered by CUSTOMER_ID
    And I create the STG_CUSTOMER_WITH_RANK stage
    And I insert by rank into the SATELLITE sat
    And I insert by rank into the SATELLITE sat
    Then the SATELLITE table should contain expected data
  ...
  @fixture.satellite
  Scenario: [SAT-RANK-MAT] Base load of a satellite with multiple timestamps in the same day in rank column partitioned by customer id loads only first rank
    Given the SATELLITE_TS table does not exist
    And the RAW_STAGE_TS table contains data
      | CUSTOMER_ID | CUSTOMER_NAME | CUSTOMER_DOB | CUSTOMER_PHONE  | LOAD_DATETIME           | SOURCE |
      | 1001        | Alice         | 1997-04-24   | 17-214-233-1214 | 1993-01-01 11:14:54.396 | *      |
      ...
      | 1003        | Chad          | 2013-02-04   | 17-214-233-1216 | 1993-01-01 11:14:54.398 | *      |
      | 1004        | Dom           | 2018-04-13   | 17-214-233-1217 | 1993-01-01 11:14:54.399 | *      |
    And I have a rank column DBTVAULT_RANK in the STG_CUSTOMER_WITH_RANK_TS stage partitioned by CUSTOMER_ID and ordered by LOAD_DATETIME
    And I create the STG_CUSTOMER_WITH_RANK_TS stage
  ...
  @fixture.satellite
  Scenario: [SAT-RANK-MAT] Incremental load of a satellite with multiple timestamps in the same day in rank column partitioned by customer id loads all records
    Given the SATELLITE_TS table does not exist
    And the RAW_STAGE_TS table contains data
      | CUSTOMER_ID | CUSTOMER_NAME | CUSTOMER_DOB | CUSTOMER_PHONE  | LOAD_DATETIME           | SOURCE |
      | 1001        | Alice         | 1997-04-24   | 17-214-233-1214 | 1993-01-01 11:14:54.396 | *      |
