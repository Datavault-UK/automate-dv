...
{%- for compare_col in compare_columns -%}
    ...
    {%- do compare_columns_processed.append("CAST({} AS STRING) AS {}".format(automate_dv.escape_column_names(compare_col), automate_dv.escape_column_names(compare_col))) -%}
...
{%- for source_col in source_columns -%}
    ...
    {%- if source_col.data_type == 'BYTES' -%}
        {%- do source_columns_processed.append("UPPER(TO_HEX({})) AS {}".format(automate_dv.escape_column_names(source_col.name), automate_dv.escape_column_names(source_col.name))) -%}
    {%- else -%}
        {%- do source_columns_processed.append("CAST({} AS STRING) AS {}".format(automate_dv.escape_column_names(source_col.name), automate_dv.escape_column_names(source_col.name))) -%}
    {%- endif -%}
