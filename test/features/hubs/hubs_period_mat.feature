Feature: [HUB-PM] Hubs Loaded using Period Materialization
  ...
  Scenario: [HUB-PM-001] Simple load of stage data into an empty hub
  ...
  @fixture.single_source_hub
  Scenario: [HUB-PM-002] Full refresh of loaded hub
    Given the HUB table does not exist
    And the RAW_STAGE table contains data
  ...
  Scenario: [HUB-PM-003] Simple load of stage data from multiple sources into an empty hub
    Given the HUB table does not exist
