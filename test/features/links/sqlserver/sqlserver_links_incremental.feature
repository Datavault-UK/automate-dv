Feature: [SQLS-LNK] Links loaded using Incremental Materialization

# Tests from snowflake_links.feature that should be moved here:
# [SQLS-LNK-04] Load a simple stage table into an empty link table
# [SQLS-LNK-06] Load a simple stage table into a populated link.
# (?) [SQLS-LNK-011] Union three staging tables to feed empty link
...
#  Scenario: [SQLS-LNK-IM-01] Load of empty stage data into a non-existent link - one cycle
...
#      | md5('1007\|\|ITA') | md5('1007') | md5('ITA') | 1993-01-02 | CRM    |
  ...
  @fixture.single_source_link
  Scenario: [SQLS-LNK-IM-02] Load of mixed stage data into a non-existent link - one cycle
    Given the LINK table does not exist
    And the RAW_STAGE table contains data
      | CUSTOMER_ID | NATION_ID | CUSTOMER_NAME | CUSTOMER_DOB | CUSTOMER_PHONE  | LOAD_DATE  | SOURCE |
      | 1003        | AUS       | Bob           | 2013-02-04   | 17-214-233-1215 | 1993-01-02 | CRM    |
  ...
  @fixture.single_source_link
  Scenario: [SQLS-LNK-IM-03] Load of mixed stage data into a non-existent link - two cycles
    Given the LINK table does not exist
    And the RAW_STAGE table contains data
      | CUSTOMER_ID | NATION_ID | CUSTOMER_NAME | CUSTOMER_DOB | CUSTOMER_PHONE  | LOAD_DATE  | SOURCE |
      | 1003        | AUS       | Bob           | 2013-02-04   | 17-214-233-1215 | 1993-01-02 | CRM    |
...
# TODO and the "And I have an empty RAW_STAGE raw stage" step fail
#  @fixture.single_source_link
#  Scenario: [SQLS-LNK-IM-04] Load of stage data + empty stage data into an empty link - two cycles
...
#      | md5('1007\|\|ITA') | md5('1007') | md5('ITA') | 1993-01-02 | CRM    |
  ...
  @fixture.single_source_link
  Scenario: [SQLS-LNK-IM-05] Load of stage data into an empty link - one cycle
    Given the LINK link is empty
    And the RAW_STAGE table contains data
      | CUSTOMER_ID | NATION_ID | CUSTOMER_NAME | CUSTOMER_DOB | CUSTOMER_PHONE  | LOAD_DATE  | SOURCE |
      | 1003        | AUS       | Bob           | 2013-02-04   | 17-214-233-1215 | 1993-01-02 | CRM    |
  ...
  @fixture.single_source_link
  Scenario: [SQLS-LNK-IM-06] Load of mixed stage data into an empty link - two cycles
    Given the LINK link is empty
    And the RAW_STAGE table contains data
      | CUSTOMER_ID | NATION_ID | CUSTOMER_NAME | CUSTOMER_DOB | CUSTOMER_PHONE  | LOAD_DATE  | SOURCE |
      | 1003        | AUS       | Bob           | 2013-02-04   | 17-214-233-1215 | 1993-01-02 | CRM    |
...
# TODO and the "And I have an empty RAW_STAGE raw stage" step fail
#  @fixture.single_source_link
#  Scenario: [SQLS-LNK-IM-07] Load of empty stage data into a populated link - one cycle
...
#      | md5('1005\|\|ITA') | md5('1005') | md5('ITA') | 1993-01-01 | CRM    |
  ...
  @fixture.single_source_link
  Scenario: [SQLS-LNK-IM-08] Load of mixed stage data into a populated link - one cycle
    Given the LINK link is already populated with data
      | CUSTOMER_NATION_PK | CUSTOMER_FK | NATION_FK  | LOAD_DATE  | SOURCE |
      | md5('1001\|\|GBR') | md5('1001') | md5('GBR') | 1993-01-01 | CRM    |
      | md5('1002\|\|POL') | md5('1002') | md5('POL') | 1993-01-01 | CRM    |
  ...
  @fixture.single_source_link
  Scenario: [SQLS-LNK-IM-09] Load of mixed stage data into a populated link - two cycles
    Given the LINK link is already populated with data
      | CUSTOMER_NATION_PK | CUSTOMER_FK | NATION_FK  | LOAD_DATE  | SOURCE |
      | md5('1001\|\|GBR') | md5('1001') | md5('GBR') | 1993-01-01 | CRM    |
      | md5('1002\|\|POL') | md5('1002') | md5('POL') | 1993-01-01 | CRM    |
