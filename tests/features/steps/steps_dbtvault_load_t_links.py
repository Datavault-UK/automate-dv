...
def step_impl(context):
    ...
    context.dbutils.drop_and_create(DATABASE, VLT_SCHEMA, "t_link_transaction_{}".format(MODE.lower()),
...
def step_impl(context):
    ...
    context.dbutils.drop_and_create(DATABASE, STG_SCHEMA, "stg_transaction_{}".format(MODE.lower()),
    ...
    context.dbutils.insert_data_from_ct(context.table, "stg_transaction_{}".format(MODE.lower()), STG_SCHEMA, context.connection)
...
def step_impl(context):
    ...
    result_df = context.dbutils.get_table_data(full_table_name=DATABASE + "." + VLT_SCHEMA + ".t_link_transaction_{}".format(MODE.lower()),
                                               binary_columns=['TRANSACTION_PK', 'CUSTOMER_PK'],
...
def step_impl(context):
    ...
    context.dbutils.insert_data_from_ct(context.table, "t_link_transaction_{}".format(MODE.lower()), VLT_SCHEMA,
                                        context.connection)


@step("the STG_TRANSACTION table has data inserted into it")
def step_impl(context):
    context.dbutils.insert_data_from_ct(context.table, "stg_transaction_{}".format(MODE.lower()), STG_SCHEMA,
                                        context.connection)
