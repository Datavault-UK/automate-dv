Feature: [DB-HUB] Hubs
  ...
  Scenario: [DB-HUB-01] Simple load of stage data into an empty hub
  ...
  @fixture.single_source_hub
  Scenario: [DB-HUB-02] Simple load of distinct stage data into an empty hub
    Given the HUB table does not exist
    And the RAW_STAGE table contains data
  ...
  @fixture.single_source_hub
  @fixture.enable_sha
  Scenario: [DB-HUB-03] Simple load of distinct stage data into an empty hub using SHA hashing
  ...
  @fixture.single_source_hub
  Scenario: [DB-HUB-04] Keys with NULL or empty values are not loaded into empty hub that does not exist
    Given the HUB hub is empty
    And the RAW_STAGE table contains data
      | CUSTOMER_ID | CUSTOMER_NAME | CUSTOMER_DOB | LOAD_DATE  | SOURCE |
      | 1001        | Alice         | 1997-04-24   | 1993-01-01 | TPCH   |
  ...
  @fixture.single_source_hub
  Scenario: [DB-HUB-05] Simple load of stage data into an empty hub
    Given the HUB hub is empty
    And the RAW_STAGE table contains data
      | CUSTOMER_ID | CUSTOMER_NAME | CUSTOMER_DOB | LOAD_DATE  | SOURCE |
      | 1001        | Alice         | 1997-04-24   | 1993-01-01 | TPCH   |
  ...
  @fixture.single_source_hub
  Scenario: [DB-HUB-06] Simple load of distinct stage data into an empty hub
    Given the HUB hub is empty
    And the RAW_STAGE table contains data
      | CUSTOMER_ID | CUSTOMER_NAME | CUSTOMER_DOB | LOAD_DATE  | SOURCE |
      | 1001        | Alice         | 1997-04-24   | 1993-01-01 | TPCH   |
  ...
  @fixture.single_source_hub
  Scenario: [DB-HUB-07] Keys with NULL or empty values are not loaded into an empty hub
    Given the HUB hub is empty
    And the RAW_STAGE table contains data
      | CUSTOMER_ID | CUSTOMER_NAME | CUSTOMER_DOB | LOAD_DATE  | SOURCE |
      | 1001        | Alice         | 1997-04-24   | 1993-01-01 | TPCH   |
  ...
  @fixture.single_source_hub
  Scenario: [DB-HUB-08] Load of stage data into a hub
  ...
  @fixture.single_source_hub
  Scenario: [DB-HUB-09] Load of distinct stage data into a hub
    Given the HUB hub is already populated with data
      | CUSTOMER_PK | CUSTOMER_ID | LOAD_DATE  | SOURCE |
      | md5('1001') | 1001        | 1993-01-01 | TPCH   |
      | md5('1002') | 1002        | 1993-01-01 | TPCH   |
  ...
  @fixture.single_source_hub
  Scenario: [DB-HUB-010] Keys with NULL or empty values are not loaded into a hub
    Given the HUB hub is already populated with data
      | CUSTOMER_PK | CUSTOMER_ID | LOAD_DATE  | SOURCE |
      | md5('1001') | 1001        | 1993-01-01 | TPCH   |
      | md5('1002') | 1002        | 1993-01-01 | TPCH   |
  ...
  Scenario: [DB-HUB-011] Union three staging tables to feed a empty hub which does not exist
    Given the HUB table does not exist
  ...
  @fixture.multi_source_hub
  Scenario: [DB-HUB-012] Keys with NULL or empty values in the union of three staging tables are not feed into an empty hub which does not exist
    Given the HUB table does not exist
    And the RAW_STAGE_PARTS table contains data
      | PART_ID | PART_NAME | PART_TYPE | PART_SIZE | PART_RETAILPRICE | LOAD_DATE  | SOURCE |
  ...
  @fixture.multi_source_hub
  Scenario: [DB-HUB-013] Union three staging tables to feed an empty hub
    Given the HUB hub is empty
    And the RAW_STAGE_PARTS table contains data
      | PART_ID | PART_NAME | PART_TYPE | PART_SIZE | PART_RETAILPRICE | LOAD_DATE  | SOURCE |
      | 1001    | Pedal     | internal  | M         | 60.00            | 1993-01-01 | PART   |
  ...
  @fixture.multi_source_hub
  Scenario: [DB-HUB-014] Union three staging tables to feed an empty hub over two cycles
    Given the HUB hub is already populated with data
      | PART_PK     | PART_ID | LOAD_DATE  | SOURCE |
      | md5('1001') | 1001    | 1993-01-01 | *      |
      | md5('1002') | 1002    | 1993-01-01 | *      |
  ...
  @fixture.multi_source_hub
  Scenario: [DB-HUB-015] Union three staging tables to feed a populated hub
    Given the HUB hub is already populated with data
      | PART_PK     | PART_ID | LOAD_DATE  | SOURCE |
      | md5('1001') | 1001    | 1993-01-01 | LINE   |
      | md5('1002') | 1002    | 1993-01-01 | LINE   |
  ...
  @fixture.multi_source_hub
  Scenario: [DB-HUB-016] Keys with a NULL or empty value in a union of three staging tables are not fed into a populated hub
    Given the HUB hub is already populated with data
      | PART_PK     | PART_ID | LOAD_DATE  | SOURCE |
      | md5('1001') | 1001    | 1993-01-01 | LINE   |
      | md5('1002') | 1002    | 1993-01-01 | LINE   |
