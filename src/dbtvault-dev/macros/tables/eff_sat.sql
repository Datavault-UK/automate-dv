...
{% else %}
    ...
    (
        ...
        WHERE TO_DATE(a.END_DATETIME) = TO_DATE('9999-12-31')
        AND a.RowNum = 1
    ),
    stage_slice AS
    (
    ...
    SELECT DISTINCT
        {{ dbtvault.alias_all(source_cols, 'slice_a') }}
    ...
    UNION
    SELECT DISTINCT
        h.{{ src_pk }}, slice_b.{{ src_ldts }}, slice_b.{{ src_eff_from}}, h.{{ src_source }}, h.{{ src_start_date }},
        slice_b.EFFECTIVE_FROM AS END_DATETIME
