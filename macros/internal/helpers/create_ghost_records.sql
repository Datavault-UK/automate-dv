...
{%- for col in columns -%}
    ...
    {%- if string_col in source_columns -%}
        ...
        {%- elif fetched_type == 'BINARY' -%}
            ...
            {%- elif hash == 'SHA' -%}
                {%- set col_sql = "CAST('{}' AS BINARY(32)) AS {}".format(fetched_string, fetched_name) -%}
            {%- else -%}
                {%- set col_sql = "CAST('{}' AS BINARY(16)) AS {}".format(fetched_string, fetched_name) -%}
            {%- endif -%}
...
SELECT
    ...
    {%- elif fetched_type == 'binary' -%}
        ...
        {%- set hash_alg = 'SHA2_256' -%}
            {%- set col_sql = "CAST(REPLICATE(CAST(CAST({} AS tinyint) AS {}), 32) AS {}) AS {}".format(fetched_string, 'binary(32)', 'binary(32)', fetched_name) -%}
        {%- else -%}
        {%- set hash_alg = 'MD5' -%}
            {%- set col_sql = "CAST(REPLICATE(CAST(CAST({} AS tinyint) AS {}), 16) AS {}) AS {}".format(fetched_string, 'binary(16)', 'binary(16)', fetched_name) -%}
        {%- endif -%}
...
{%- macro databricks__create_ghost_records(source_model, source_columns, hashes, record_source) -%}
...
{%- for col in columns -%}
    ...
    {%- if string_col in source_columns -%}
        ...
        {# Ensure record source is not set as just VARCHAR #}
        {%- if fetched_name == record_source -%}
            {%- set col_sql = "CAST('DBTVAULT_SYSTEM' AS VARCHAR(100)) AS {}".format(fetched_name) -%}
