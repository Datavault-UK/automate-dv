...
{%- macro default__hub(
    ...
    {%- set source_cols = dbtvault.expand_column_list(
        columns = [src_pk, src_nk, src_extra_columns, src_ldts, src_source]
    ) -%}
    ...
    {%- for src in source_model -%}
        ...
        row_rank_ {{ source_number }} AS (
            ...
            WHERE
                ...
                qualify ROW_NUMBER = 1 {%- set ns.last_cte = "row_rank_{}".format(source_number) %}
        ),
        {{ "\n" if not loop.last }}
    {% endfor -%}

    {% if stage_count > 1 %}
        stage_union AS ({%- for src in source_model %}
        SELECT
            *
        FROM
            row_rank_ {{ loop.index | string }}
    ...
    {%- if model.config.materialized == 'vault_insert_by_period' %}
        stage_mat_filter AS (
            ...
            FROM
                {{ ns.last_cte }}
            WHERE
                __PERIOD_FILTER__ {%- set ns.last_cte = "stage_mat_filter" %}
        ),
        ...
        stage_mat_filter AS (
            ...
            FROM
                {{ ns.last_cte }}
            WHERE
                __RANK_FILTER__ {%- set ns.last_cte = "stage_mat_filter" %}
        ),
    {%- endif -%}
    ...
    {%- if stage_count > 1 %}
        row_rank_union AS (
            ...
            WHERE
                ...
                qualify row_rank_number = 1 {%- set ns.last_cte = "row_rank_union" %}
        ),
    {% endif %}
    ...
    records_to_insert AS (
        ...
        FROM
            {{ ns.last_cte }} AS A {%- if dbtvault.is_any_incremental() %}
                LEFT JOIN {{ this }} AS d
