Feature: [SQLS-SAT-PM-P] Satellites Loaded using Period Materialization for provided date ranges
  ...
  Scenario: [SQLS-SAT-PM-P-01] Satellite load over several daily cycles with insert_by_period into non-existent satellite, with date range.
  ...
  @fixture.satellite_cycle
  Scenario: [SQLS-SAT-PM-P-02] Satellite load over several daily cycles with insert_by_period into empty satellite, with date range.
  ...
  @fixture.satellite_cycle
  Scenario: [SQLS-SAT-PM-P-03] Satellite load over several daily cycles with insert_by_period into populated satellite, with partial duplicates and date range
    Given the RAW_STAGE stage is empty
  ...
  @fixture.satellite_cycle
  Scenario: [SQLS-SAT-PM-P-04] Satellite load over several daily cycles with insert_by_period into populated satellite, with all duplicates and date range.
    Given the RAW_STAGE stage is empty
    And the SATELLITE sat is already populated with data
      | CUSTOMER_PK | HASHDIFF                             | CUSTOMER_NAME | CUSTOMER_DOB | EFFECTIVE_FROM | LOAD_DATE  | SOURCE |
      | md5('1001') | md5('1990-02-03\|\|1001\|\|ALBERT')  | Albert        | 1990-02-03   | 2019-05-04     | 2019-05-04 | *      |
