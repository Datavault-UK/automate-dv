Feature: [SQLS-MAS-2CD-B] Multi Active Satellites
  ...
  Scenario: [SQLS-MAS-2CD-B-01] Load data into a non-existent multi-active satellite, where some customers have the same phone number but different extensions and others have different phone numbers but the same extensions
  ...
  @fixture.multi_active_satellite
  Scenario: [SQLS-MAS-2CD-B-02] Load duplicated data into a non-existent multi-active satellite
    Given the MULTI_ACTIVE_SATELLITE_TWO_CDK table does not exist
    And the RAW_STAGE_TWO_CDK table contains data
      | CUSTOMER_ID | CUSTOMER_NAME | CUSTOMER_PHONE  | EXTENSION | LOAD_DATE  | SOURCE |
      | 1001        | Alice         | 17-214-233-1214 | 12301     | 1993-01-01 | *      |
  ...
  @fixture.multi_active_satellite
  Scenario: [SQLS-MAS-2CD-B-03] Load data into an empty multi-active satellite
  ...
  @fixture.multi_active_satellite
  Scenario: [SQLS-MAS-2CD-B-04] Load duplicated data into an empty multi-active satellite
    Given the MULTI_ACTIVE_SATELLITE_TWO_CDK ma_sat is empty
    And the RAW_STAGE_TWO_CDK table contains data
      | CUSTOMER_ID | CUSTOMER_NAME | CUSTOMER_PHONE  | EXTENSION | LOAD_DATE  | SOURCE |
      | 1001        | Alice         | 17-214-233-1214 | 12301     | 1993-01-01 | *      |
  ...
  @fixture.multi_active_satellite
  Scenario: [SQLS-MAS-2CD-B-05] Load data into an empty multi-active satellite where some records have NULL CDK(s) or Attribute(s)
    Given the MULTI_ACTIVE_SATELLITE_TWO_CDK ma_sat is empty
    And the RAW_STAGE_TWO_CDK table contains data
      | CUSTOMER_ID | CUSTOMER_NAME | CUSTOMER_PHONE  | EXTENSION | LOAD_DATE  | SOURCE |
      | 1001        | Alice         | 17-214-233-1214 | 12301     | 1993-01-01 | *      |
