...
WITH source_data AS (
    ...
    {% if dbtvault.is_any_incremental() %}
        ,COUNT(DISTINCT {{ dbtvault.prefix([src_hashdiff], 's', alias_target='source') }}, {{ dbtvault.prefix(cdk_cols, 's') }} )
    ...
    FROM (
        ...
        {%- endif %}
        {# Select the earliest record for each pk + hashdiff + cdk combination #}
        ,RANK() OVER (PARTITION BY {{ dbtvault.prefix([src_pk], 'a') }}, {{ dbtvault.prefix([src_hashdiff], 'a', alias_target='source') }}, {{ dbtvault.prefix(cdk_cols, 'a') }}
            ORDER BY {{ dbtvault.prefix([src_ldts], 'a') }} ASC) AS source_rank
...
WITH source_data_filtered AS (
    ...
    {% if dbtvault.is_any_incremental() %}
    ,RANK() OVER (PARTITION BY {{ dbtvault.prefix([src_pk], 'a') }}, {{ dbtvault.prefix([src_hashdiff], 'a', alias_target='source') }}, {{ dbtvault.prefix(cdk_cols, 'a') }}
        ORDER BY {{ dbtvault.prefix([src_ldts], 'a') }} ASC) as r
    {%- endif %}
    FROM {{ ref(source_model) }} AS a
    WHERE {{ dbtvault.prefix([src_pk], 'a') }} IS NOT NULL
    {%- for child_key in src_cdk %}
...
source_data AS (
    ...
    FROM (
        SELECT s.*
        ,RANK() OVER (PARTITION BY {{ dbtvault.prefix([src_pk], 's') }}, {{ dbtvault.prefix([src_hashdiff], 's', alias_target='source') }}, {{ dbtvault.prefix(cdk_cols, 's') }}
            ORDER BY {{ dbtvault.prefix([src_ldts], 's') }} ASC) AS source_rank
