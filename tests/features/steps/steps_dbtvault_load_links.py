...
def step_impl(context):
    ...
    context.dbutils.insert_data_from_ct(context.table, "test_stg_customer_links", STG_SCHEMA)
...
def step_impl(context):
    ...
    context.dbutils.insert_data_from_ct(context.table, "test_link_customer_nation", VLT_SCHEMA)
...
def step_impl(context):
    ...
    result_df = context.dbutils.get_table_data(full_table_name=DATABASE + VLT_SCHEMA + ".test_link_customer_nation",
                                               binary_columns=['CUSTOMER_NATION_PK', 'CUSTOMER_FK', 'NATION_FK'],
...
def step_impl(context):
    ...
    context.dbutils.drop_and_create(DATABASE, STG_SCHEMA, "test_stg_sap_customer",
                                    ["CUSTOMER_ID VARCHAR(38)", "NATION_ID VARCHAR(38)", "CUSTOMER_NAME VARCHAR(25)",
                                     "CUSTOMER_DOB DATE", "CUSTOMER_PHONE VARCHAR(15)", "LOADDATE DATE",
                                     "SOURCE VARCHAR(15)"], connection=context.connection)
    context.dbutils.insert_data_from_ct(context.table, "test_stg_sap_customer", STG_SCHEMA)
...
def step_impl(context):
    ...
    context.dbutils.drop_and_create(DATABASE, STG_SCHEMA, "test_stg_crm_customer",
                                    ["CUSTOMER_REF VARCHAR(38)", "NATION_KEY VARCHAR(38)", "CUSTOMER_NAME VARCHAR(25)",
                                     "CUSTOMER_DOB DATE", "CUSTOMER_PHONE VARCHAR(15)", "LOADDATE DATE",
                                     "SOURCE VARCHAR(15)"], connection=context.connection)
    context.dbutils.insert_data_from_ct(context.table, "test_stg_crm_customer", STG_SCHEMA)
...
def step_impl(context):
    ...
    context.dbutils.drop_and_create(DATABASE, STG_SCHEMA, "test_stg_web_customer",
                                    ["CUSTOMER_REF VARCHAR(38)", "NATION_KEY VARCHAR(38)", "CUSTOMER_NAME VARCHAR(25)",
                                     "CUSTOMER_DOB DATE", "CUSTOMER_PHONE VARCHAR(15)", "LOADDATE DATE",
                                     "SOURCE VARCHAR(15)"], connection=context.connection)
    context.dbutils.insert_data_from_ct(context.table, "test_stg_web_customer", STG_SCHEMA)
