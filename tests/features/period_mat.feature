Feature: Period Materialization
  ...
  @fixture.satellite_cycle
  Scenario: [SAT-CYCLE-PERIOD-MAT] Satellite load over several cycles with insert_by_period into non-existent satellite, with date range.
    Given the SATELLITE table does not exist
    And the RAW_STAGE table contains data
      | CUSTOMER_ID | CUSTOMER_NAME | CUSTOMER_DOB | EFFECTIVE_FROM | LOADDATE   | SOURCE |
      | 1001        | Albert        | 1990-02-03   | 2019-05-04     | 2019-05-04 | *      |
      ...
      | 1011        | Karen         | 1978-06-16   | 2019-05-07     | 2019-05-07 | *      |
    And I hash the stage
    And I use insert_by_period to load the SATELLITE sat with date range: 2019-05-05 to 2019-05-06
    And I use insert_by_period to load the SATELLITE sat with date range: 2019-05-05 to 2019-05-06
  ...
  @fixture.satellite_cycle
  Scenario: [SAT-CYCLE-PERIOD-MAT] Satellite load over several cycles with insert_by_period into empty satellite, with date range.
    Given the RAW_STAGE stage is empty
    And the SATELLITE sat is empty
    When the RAW_STAGE is loaded
      | CUSTOMER_ID | CUSTOMER_NAME | CUSTOMER_DOB | EFFECTIVE_FROM | LOADDATE   | SOURCE |
      ...
      | 1011        | Karen         | 1978-06-16   | 2019-05-07     | 2019-05-07 | *      |
    And I hash the stage
    And I use insert_by_period to load the SATELLITE sat with date range: 2019-05-04 to 2019-05-06
  ...
  @fixture.satellite_cycle
  Scenario: [SAT-CYCLE-PERIOD-MAT] Satellite load over several cycles with insert_by_period into populated satellite, with all duplicates and date range.
    Given the RAW_STAGE stage is empty
    Given the SATELLITE sat is already populated with data
      | CUSTOMER_PK | HASHDIFF                             | CUSTOMER_NAME | CUSTOMER_DOB | EFFECTIVE_FROM | LOADDATE   | SOURCE |
      | md5('1001') | md5('1990-02-03\|\|1001\|\|ALBERT')  | Albert        | 1990-02-03   | 2019-05-04     | 2019-05-04 | *      |
    ...
    When the RAW_STAGE is loaded
      ...
      | 1011        | Karen         | 1978-06-16   | 2019-05-07     | 2019-05-07 | *      |
    And I hash the stage
    And I use insert_by_period to load the SATELLITE sat with date range: 2019-05-04 to 2019-05-05
  ...
  @fixture.satellite_cycle
  Scenario: [SAT-CYCLE-PERIOD-MAT] Satellite load over several cycles with insert_by_period into populated satellite, with partial duplicates and date range
    Given the RAW_STAGE stage is empty
    Given the SATELLITE sat is already populated with data
      | CUSTOMER_PK | HASHDIFF                             | CUSTOMER_NAME | CUSTOMER_DOB | EFFECTIVE_FROM | LOADDATE   | SOURCE |
      | md5('1001') | md5('1990-02-03\|\|1001\|\|ALBERT')  | Albert        | 1990-02-03   | 2019-05-04     | 2019-05-04 | *      |
