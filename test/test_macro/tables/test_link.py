...
# class TestLinkMacro(TestCase):
#     maxDiff = None
#
#     def test_link_macro_correctly_generates_sql_for_single_source(self):
#         process_logs = self.dbt_test_utils.run_dbt_models(model_names=[self.current_test_name], full_refresh=True)
...
#         process_logs_first_run = self.dbt_test_utils.run_dbt_models(mode='run', model_names=[self.current_test_name],
#                                                                     full_refresh=True)
#         process_logs_inc_run = self.dbt_test_utils.run_dbt_models(mode='run', model_names=[self.current_test_name])
#         actual_sql = self.dbt_test_utils.retrieve_compiled_model(self.current_test_name)
#         expected_sql = self.dbt_test_utils.retrieve_expected_sql(self.current_test_name)
#
