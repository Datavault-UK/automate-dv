...
class DBTVAULTHarnessUtils:
    ...
    def __init__(self, model_directory=None):
        ...
        if model_directory:
            self.compiled_model_path = test.COMPILED_TESTS_DBT_ROOT / model_directory
            self.expected_sql_file_path = test.EXPECTED_OUTPUT_FILE_ROOT / model_directory

        os.environ['TARGET'] = self.target

        if self.target in test.AVAILABLE_TARGETS:
    ...
    def clean_target():
        ...
        target = test.TESTS_DBT_ROOT / 'target'
