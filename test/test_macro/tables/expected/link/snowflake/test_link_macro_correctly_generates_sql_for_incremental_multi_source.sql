WITH row_rank_1 AS (SELECT rr.CUSTOMER_PK,
                           rr.ORDER_FK,
                           rr.BOOKING_FK,
                           rr.LOAD_DATE,
                           rr.RECORD_SOURCE,
               ...
               ORDER BY rr.LOAD_DATE
                    ...
                    FROM
   [DATABASE_NAME].[SCHEMA_NAME].raw_source AS rr
    QUALIFY row_number = 1
    )
   , row_rank_2 AS (
SELECT rr.CUSTOMER_PK, rr.ORDER_FK, rr.BOOKING_FK, rr.LOAD_DATE, rr.RECORD_SOURCE, ROW_NUMBER() OVER(
    PARTITION BY rr.CUSTOMER_PK
    ORDER BY rr.LOAD_DATE
    ) AS row_number
