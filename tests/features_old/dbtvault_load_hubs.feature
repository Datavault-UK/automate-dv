...
# -----------------------------------------------------------------------------
  ...
  Scenario: [SINGLE-SOURCE] Keys with NULL or empty values are not loaded into an existing empty hub
    Given there is an empty TEST_HUB_CUSTOMER_HUBS table
    And there are records in the TEST_STG_CUSTOMER table
      | CUSTOMER_ID | CUSTOMER_NAME | CUSTOMER_DOB | LOADDATE   | SOURCE |
      | 1001        | Alice         | 1997-04-24   | 1993-01-01 | TPCH   |
...
# -----------------------------------------------------------------------------
  ...
  Scenario: [UNION] Keys with a NULL or empty value in a union of three staging tables are not fed into a populated hub.
    Given there are records in the TEST_HUB_PARTS table
      | PART_PK     | PART_ID | LOADDATE   | SOURCE |
      | md5('1001') | 1001    | 1993-01-01 | LINE   |
      | md5('1002') | 1002    | 1993-01-01 | LINE   |
