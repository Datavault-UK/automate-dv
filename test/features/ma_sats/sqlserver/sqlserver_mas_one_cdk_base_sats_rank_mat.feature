Feature: [SF-MAS-RM] Multi Active Satellites
  ...
  @fixture.multi_active_satellite_sqlserver
  Scenario: [SF-MAS-RM-008] Base load of a multi-active satellite with multiple timestamps in the same day in rank column only loads first rank
    Given the MULTI_ACTIVE_SATELLITE_TS table does not exist
    And the RAW_STAGE_TS table contains data
      | CUSTOMER_ID | CUSTOMER_NAME | CUSTOMER_PHONE  | LOAD_DATETIME           | SOURCE |
  ...
  @fixture.multi_active_satellite_sqlserver
  Scenario: [SF-MAS-RM-009] Incremental load of a multi-active satellite with multiple timestamps in the same day in rank column loads records without duplicates
    Given the MULTI_ACTIVE_SATELLITE_TS table does not exist
    And the RAW_STAGE_TS table contains data
      | CUSTOMER_ID | CUSTOMER_NAME | CUSTOMER_PHONE  | LOAD_DATETIME           | SOURCE |
      | 1001        | Alice         | 17-214-233-1214 | 1993-01-01 11:14:54.380 | *      |
  ...
  @fixture.multi_active_satellite_sqlserver
  Scenario: [SF-MAS-RM-010] Incremental load of a multi-active satellite with multiple timestamps in the same day in rank column partitioned by customer id loads all records
    Given the MULTI_ACTIVE_SATELLITE_TS table does not exist
    And the RAW_STAGE_TS table contains data
      | CUSTOMER_ID | CUSTOMER_NAME | CUSTOMER_PHONE  | LOAD_DATETIME           | SOURCE |
      | 1001        | Alice         | 17-214-233-1214 | 1993-01-01 11:14:54.396 | *      |
  ...
  @fixture.multi_active_satellite_sqlserver
  Scenario: [SF-MAS-RM-011] Incremental load of a multi-active satellite with multiple sets of records per load, loads all records
    Given the MULTI_ACTIVE_SATELLITE table does not exist
    And the RAW_STAGE table contains data
      | CUSTOMER_ID | CUSTOMER_NAME | CUSTOMER_PHONE  | LOAD_DATE  | SOURCE |
      | 1001        | Alice         | 17-214-233-1214 | 1993-01-01 | *      |
  ...
  Scenario: [SF-MAS-RM-012] Loading in cycles: identical data into a multi-active satellite with one value in rank column loads first rank only and once only
...
#  @fixture.multi_active_satellite_cycle_sqlserver
#  Scenario: [SF-MAS-RM-001] Loading in cycles: waterlevel + changed payload in old record but identical to the currently valid record + partially overlapping data into a multi-active satellite with one value in rank column
#    Given the RAW_STAGE stage is empty
#    And the MULTI_ACTIVE_SATELLITE ma_sat is empty
#
#    # ================ LOAD 1 ===================
...
#  @fixture.multi_active_satellite_cycle_sqlserver
#  Scenario: [SF-MAS-RM-001] Loading in cycles: waterlevel + new payload in old record + partially overlapping data into a multi-active satellite with one value in rank column
#    Given the RAW_STAGE stage is empty
#    And the MULTI_ACTIVE_SATELLITE ma_sat is empty
#
#    # ================ LOAD 1 ===================
