...
def step_impl(context):
    ...
    context.testdata.drop_and_create("DV_PROTOTYPE_DB", "SRC_TEST_STG", "test_stg_customer_hubs",
                                     ["CUSTOMER_ID VARCHAR(38)", "CUSTOMER_NAME VARCHAR(25)",
                                      "CUSTOMER_DOB DATE",
...
def step_impl(context):
    ...
    os.system("dbt run --full-refresh --models snow_vault.features.load_hubs.*")


@step("I run the dbt hub load")
def step_impl(context):
    os.chdir(DBT_ROOT)

    os.system("dbt run --models snow_vault.features.load_hubs.*")
...
def step_impl(context):
    ...
    context.testdata.drop_and_create("DV_PROTOTYPE_DB", "SRC_TEST_STG", "test_stg_parts",
                                     ["PART_ID VARCHAR(38)", "PART_NAME VARCHAR(60)",
                                      "PART_TYPE VARCHAR(10)", "PART_SIZE VARCHAR(5)", "PART_RETAILPRICE DOUBLE",
                                      "LOADDATE DATE", "SOURCE VARCHAR(4)"],
                                     materialise="table")
...
def step_impl(context):
    ...
    context.testdata.drop_and_create("DV_PROTOTYPE_DB", "SRC_TEST_STG", "test_stg_supplier",
                                     ["PART_PK BINARY(16)", "SUPPLIER_PK BINARY(16)", "PART_ID VARCHAR(38)",
                                      "AVAILQTY INT", "SUPPLYCOST DOUBLE",
                                      "LOADDATE DATE", "SOURCE VARCHAR(4)"],
                                     materialise="table")
...
def step_impl(context):
    ...
    context.testdata.drop_and_create("DV_PROTOTYPE_DB", "SRC_TEST_STG", "stg_lineitem",
                                     ["ORDER_PK BINARY(16)", "PART_PK BINARY(16)", "SUPPLIER_PK BINARY(16)",
                                      "PART_ID VARCHAR(38)", "LINENUMBER NUMBER(38)", "QUANTITY INT",
...
def step_impl(context):
    ...
    os.system("dbt run --models snow_vault.features.load_hubs.unions.*")
...
def step_impl(context):
    ...
    os.system("dbt run --full-refresh --models snow_vault.features.load_hubs.unions.*")
