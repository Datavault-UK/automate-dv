Feature: [HUB-RM] Hubs Loaded using Rank Materialization
  ...
  Scenario: [HUB-RM-01] Simple load of stage data into an empty hub
  ...
  @fixture.single_source_hub
  Scenario: [HUB-RM-02] Full refresh of loaded hub
  ...
  Scenario: [HUB-RM-03] Simple load of stage data from multiple sources into an empty hub
    Given the HUB table does not exist
    ...
    And I have a rank column AUTOMATE_DV_RANK in the STG_PARTS stage partitioned by PART_ID and ordered by LOAD_DATE
    ...
    And I have a rank column AUTOMATE_DV_RANK in the STG_SUPPLIER stage partitioned by PART_ID and ordered by LOAD_DATE
    ...
    And I have a rank column AUTOMATE_DV_RANK in the STG_LINEITEM stage partitioned by PART_ID and ordered by LOAD_DATE
  ...
  Scenario: [HUB-RM-04] Simple load of stage data into an empty hub using insert-by-rank
    Given the HUB table does not exist
    And the RAW_STAGE table contains data
