Feature: Hubs Loaded using Rank Materialization
  ...
  Scenario: [H-020] Simple load of stage data into an empty hub
  ...
  @fixture.single_source_hub
  Scenario: [H-021] Full refresh of loaded hub
    Given the HUB table does not exist
    And the RAW_STAGE table contains data
  ...
  Scenario: [H-022] Simple load of stage data from multiple sources into an empty hub
    Given the HUB table does not exist
