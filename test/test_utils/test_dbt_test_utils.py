...
def test_calc_hash_without_hashing_for_single_column(dbt_test_utils):
    columns = Series(['1000'])

    expected_columns = Series(['1000'])
    actual_columns = dbt_test_utils.calc_hash(columns)

    assert expected_columns.equals(actual_columns)


def test_calc_hash_without_hashing_for_multiple_column(dbt_test_utils):
...
def test_calc_hash_for_single_md5(dbt_test_utils):
    columns = Series(["md5('1000')"])

    expected_hashes = Series(['A9B7BA70783B617E9998DC4DD82EB3C5'])
    actual_hashes = dbt_test_utils.calc_hash(columns)

    assert expected_hashes.equals(actual_hashes)


def test_calc_hash_for_multiple_md5(dbt_test_utils):
...
def test_calc_hash_for_single_sha256(dbt_test_utils):
...
def test_calc_hash_for_multiple_sha256(dbt_test_utils):
