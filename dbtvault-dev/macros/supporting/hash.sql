...
{%- macro greenplum__hash(columns, alias, is_hashdiff) -%}
...
{%- if columns is string -%}
    ...
    {%- else -%}
        {%- set escaped_column_str = dbtvault.escape_column_names(column_str) -%}
    {%- endif -%}
    {{- "CAST(({}({})) AS TEXT) AS {}".format(hash_alg, standardise | replace('[EXPRESSION]', escaped_column_str), dbtvault.escape_column_names(alias)) | indent(4) -}}
...
{%- else -%}
    ...
    {%- for column in columns -%}
        ...
        {%- if loop.last -%}
            ...
            {% if is_hashdiff %}
                {{- "\n)) AS TEXT) AS {}".format(dbtvault.escape_column_names(alias)) -}}
            {%- else -%}
                {{- "\n), '{}')) AS TEXT) AS {}".format(all_null | join(""), dbtvault.escape_column_names(alias)) -}}
            {%- endif -%}
        {%- else -%}
