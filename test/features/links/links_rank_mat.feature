Feature: [LNK-RM] Links Loaded using Rank Materialization
  ...
  Scenario: [LNK-RM-01] Load a simple stage table into a non-existent link table
  ...
  Scenario: [LNK-RM-02] Union three staging tables to feed empty link
    ...
    And I have a rank column AUTOMATE_DV_RANK in the STG_SAP stage partitioned by CUSTOMER_ID and ordered by LOAD_DATE
    ...
    And I have a rank column AUTOMATE_DV_RANK in the STG_CRM stage partitioned by CUSTOMER_ID and ordered by LOAD_DATE
    ...
    And I have a rank column AUTOMATE_DV_RANK in the STG_WEB stage partitioned by CUSTOMER_ID and ordered by LOAD_DATE
  ...
  Scenario: [LNK-RM-03] Load a simple stage table into a non-existent link table, rank load
    Given the LINK table does not exist
    And the RAW_STAGE table contains data
      | CUSTOMER_ID | NATION_ID | CUSTOMER_NAME | CUSTOMER_DOB | CUSTOMER_PHONE  | LOAD_DATE  | SOURCE |
      | 1001        | GBR       | Alice         | 1997-04-24   | 17-214-233-1214 | 1993-01-01 | CRM    |
