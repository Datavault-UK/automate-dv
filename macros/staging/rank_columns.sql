            ...
            {%- if columns [col] is mapping and columns [col].partition_by and columns [col].order_by -%}
                ...
                {%- else -%}
                    {%- if execute -%}
                        {%- do exceptions.raise_compiler_error(
                            'If dense_rank is provided, it must be true or false, not {}'.format(dense_rank)
                        ) -%}
                    {% endif %}
                {%- endif -%}
                ...
                {%- if dbtvault.is_list(order_by) -%}
                    ...
                    {% for order_by_col in order_by %}
                        {%- if order_by_col is mapping %}
                            {%- set column_name,
                            direction = order_by_col.items() | first -%}
                ...
                {%- else -%}
                    {%- if order_by is mapping %}
                        {%- set column_name,
                        direction = order_by.items() | first -%}
                    {%- else -%}
                ...
                {{- "{} OVER (PARTITION BY {} ORDER BY {}) AS {}".format(
                    rank_type,
                    partition_by_str,
                    order_by_str,
                    dbtvault.escape_column_names(col)
                ) | indent(4) -}}
            {%- endif -%}
