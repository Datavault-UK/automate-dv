Feature: [SQLS-TLK-PM] Transactional Links using Period Materialization
  ...
  @fixture.t_link
  Scenario: [SQLS-TLK-PM-01] Load an a non-existent Transactional Link with the period materialisation
  ...
  @fixture.t_link
  Scenario: [SQLS-TLK-PM-02] Load an empty Transactional Link with the period materialisation
    Given the T_LINK table does not exist
    And the RAW_STAGE table contains data
      | CUSTOMER_ID | ORDER_ID | TRANSACTION_NUMBER | TRANSACTION_DATE | TYPE | AMOUNT   | LOAD_DATE  | SOURCE |
      | 1234        | 4321     | 12345678           | 2019-09-19       | DR   | 2340.50  | 2019-09-21 | SAP    |
  ...
  @fixture.t_link
  Scenario: [SQLS-TLK-PM-03] Load a populated Transactional Link with the period materialisation
