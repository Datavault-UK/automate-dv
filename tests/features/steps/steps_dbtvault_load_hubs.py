...
def step_impl(context, model_name):
    ...
    os.system("dbt run --models +{}_{}".format(model_name.lower(), MODE.lower()))
...
def step_impl(context):
    ...
    context.dbutils.drop_and_create(DATABASE, STG_SCHEMA, "test_stg_customer_hubs_{}".format(MODE.lower()),
...
def step_impl(context):
    context.dbutils.create_schema(DATABASE, STG_SCHEMA, context.connection)
    context.dbutils.drop_and_create(DATABASE, STG_SCHEMA, "test_stg_parts_{}".format(MODE.lower()),
                                    ["PART_ID VARCHAR(38)", "PART_NAME VARCHAR(60)", "PART_TYPE VARCHAR(10)",
                                     "PART_SIZE VARCHAR(5)", "PART_RETAILPRICE DOUBLE", "LOADDATE DATE",
                                     "SOURCE VARCHAR(4)"], connection=context.connection)
    context.dbutils.insert_data_from_ct(context.table, "test_stg_parts_{}".format(MODE.lower()), STG_SCHEMA, context.connection)


@step("there are records in the TEST_STG_SUPPLIER table")
def step_impl(context):
    context.dbutils.create_schema(DATABASE, STG_SCHEMA, context.connection)
    context.dbutils.drop_and_create(DATABASE, STG_SCHEMA, "test_stg_supplier_{}".format(MODE.lower()),
                                    ["PART_ID VARCHAR(38)", "SUPPLIER_ID VARCHAR(38)", "AVAILQTY INT",
                                     "SUPPLYCOST DOUBLE", "LOADDATE DATE", "SOURCE VARCHAR(4)"],
                                    connection=context.connection)
    context.dbutils.insert_data_from_ct(context.table, "test_stg_supplier_{}".format(MODE.lower()), STG_SCHEMA, context.connection)


@step("there are records in the TEST_STG_LINEITEM table")
def step_impl(context):
    context.dbutils.create_schema(DATABASE, STG_SCHEMA, context.connection)
    context.dbutils.drop_and_create(DATABASE, STG_SCHEMA, "test_stg_lineitem_{}".format(MODE.lower()),
