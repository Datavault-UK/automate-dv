Feature: [SF-HUB-RM] Hubs Loaded using Rank Materialization
  ...
  Scenario: [SF-HUB-RM-01] Simple load of stage data into an empty hub
  ...
  @fixture.single_source_hub
  Scenario: [SF-HUB-RM-02] Full refresh of loaded hub
    Given the HUB table does not exist
    And the RAW_STAGE table contains data
  ...
  Scenario: [SF-HUB-RM-03] Simple load of stage data from multiple sources into an empty hub
    Given the HUB table does not exist
