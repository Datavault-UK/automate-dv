...
{% set sat_columns = 'CAST(stg.REGION_HASHDIFF AS BINARY(16)) AS REGION_HASHDIFF, CAST(stg.REGION_PK AS BINARY(16)) AS REGION_PK, CAST(stg.REGION_NAME AS VARCHAR) AS REGION_NAME, CAST(stg.REGION_COMMENT AS VARCHAR) AS REGION_COMMENT, CAST(stg.LOADDATE AS DATE) AS LOADDATE, CAST(stg.EFFECTIVE_FROM AS DATE) AS EFFECTIVE_FROM, CAST(stg.SOURCE AS VARCHAR) AS SOURCE' %}
