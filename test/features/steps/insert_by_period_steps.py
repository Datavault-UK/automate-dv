...
@step("I insert by period into the {model_name} {vault_structure} by {period} with LDTS {timestamp_field}")
def load_table(context, model_name, vault_structure, period, timestamp_field):
    metadata = {"source_model": context.processed_stage_name,
                **context.vault_structure_columns[model_name]}
...
@step("if I insert by period into the {model_name} {vault_structure} by {period} with LDTS {timestamp_field} "
      "this will fail with \"{error_message}\" error")
def load_table(context, model_name, vault_structure, period, timestamp_field, error_message):
    metadata = {"source_model": context.processed_stage_name,
                **context.vault_structure_columns[model_name]}
