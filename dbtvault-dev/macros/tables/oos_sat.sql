{%- macro oos_sat(src_pk, src_hashdiff, src_payload, src_eff, src_ldts, src_source, source_model, out_of_sequence=None) -%}
    ...
    {{- adapter.dispatch('oos_sat', 'dbtvault')(src_pk=src_pk, src_hashdiff=src_hashdiff,
...
latest_records AS (
    SELECT {{ dbtvault.prefix(rank_cols, 'c', alias_target='target') }},
        RANK() OVER (PARTITION BY {{ dbtvault.prefix([src_pk], 'c') }}
        ORDER BY {{ dbtvault.prefix([src_ldts], 'c') }} DESC) AS latest
