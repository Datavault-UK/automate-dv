...
{%- macro default__bridge(
    ...
    {%- if dbtvault.is_any_incremental() %}
        ...
        overlap AS (
            {{ dbtvault.bridge_overlap_and_new_rows(
                src_pk,
                bridge_walk,
                'overlap_pks',
                'overlap_as_of'
    ...
    new_rows AS (
                    ...
                    SELECT
                        *,
                        ROW_NUMBER() over (
                            ...
                            ORDER BY
                                {% for bridge_step in bridge_walk.keys() -%}
                                    {%- set bridge_load_date = dbtvault.escape_column_names(
                                        bridge_walk [bridge_step] ['bridge_load_date']
                                    ) -%}
                                    {{ bridge_load_date }} DESC {%- if not loop.last %},
                                    {% endif -%}
                                {%- endfor %}
                        ) AS row_num
                    FROM
        ...
        bridge AS (
            SELECT
                ...
                {%- endfor -%}
            FROM
                candidate_rows AS C {%- for bridge_step in bridge_walk.keys() -%}
                    {%- set bridge_end_date = dbtvault.escape_column_names(
                        bridge_walk [bridge_step] ['bridge_end_date']
                    ) %}
                    {% if loop.first -%}
                    WHERE
                    {%- else -%}
                    ...
                    {{ dbtvault.cast_date(dbtvault.prefix([bridge_end_date], 'c')) }} = {{ dbtvault.cast_date(
                        max_datetime,
                        true,
                        false
                    ) }}
