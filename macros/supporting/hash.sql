...
{%- if is_hashdiff is none -%} {%- set is_hashdiff = false -%} {%- endif -%}

{{-
    adapter.dispatch("dv_hash", "automate_dv")(
        columns=columns, alias=alias, is_hashdiff=is_hashdiff, columns_to_escape=columns_to_escape
    )
-}}
...
{%- if is_hashdiff and automate_dv.is_list(columns) -%} {%- set columns = columns | sort -%} {%- endif -%}
...
{{ hash_alg | replace("[HASH_STRING_PLACEHOLDER]", standardise | replace("[EXPRESSION]", column_str)) }}
as {{ alias | indent(4) }}
...
{%- if automate_dv.is_something(columns_to_escape) -%}
{%- if column in columns_to_escape -%} {%- set column = automate_dv.escape_column_name(column) -%} {%- endif -%}
{%- endif -%}
