...
order_actual_data AS (
    SELECT CAST(CUSTOMER_FK AS STRING) AS CUSTOMER_FK, CAST(NATION_FK AS STRING) AS NATION_FK, CAST(CUSTOMER_NATION_PK AS STRING) AS CUSTOMER_NATION_PK, CAST(LOAD_DATE AS STRING) AS LOAD_DATE, CAST(SOURCE AS STRING) AS SOURCE
    FROM actual_data
    ORDER BY CUSTOMER_NATION_PK, CUSTOMER_FK,  NATION_FK, CUSTOMER_PK, LOAD_DATE, SOURCE
),
