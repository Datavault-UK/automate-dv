...
{# POSTGRES #}
{# Same as default except with a minor change to quote usage of unique_id  #}
{#  #}
...
duplicates_not_in_actual AS (
    SELECT {{ columns_string }}
    FROM duplicates_expected
    WHERE {{ unique_id_quoted }} NOT IN (SELECT {{ unique_id_quoted }} FROM duplicates_actual)
),
duplicates_not_in_expected AS (
    SELECT {{ columns_string }}
    FROM duplicates_actual
    WHERE {{ unique_id_quoted }} NOT IN (SELECT {{ unique_id_quoted }} FROM duplicates_expected)
),
compare AS (
    SELECT {{ columns_string }},
