Feature: [SF-MAS-RM] Multi Active Satellites
  ...
  Scenario: [SF-MAS-RM-01] Base load of a multi-active satellite with one value in rank column loads first rank
  ...
  @fixture.multi_active_satellite
  Scenario: [SF-MAS-RM-02] Base load of a multi-active satellite with one value in rank column excludes NULL PKs and loads first rank,
    Given the MULTI_ACTIVE_SATELLITE table does not exist
    And the RAW_STAGE table contains data
      | CUSTOMER_ID | CUSTOMER_NAME | CUSTOMER_PHONE  | LOAD_DATE  | SOURCE |
      | 1001        | Alice         | 17-214-233-1214 | 1993-01-01 | *      |
  ...
  @fixture.multi_active_satellite
  Scenario: [SF-MAS-RM-03] Incremental load of a multi-active satellite with one value in rank column loads all records
    Given the MULTI_ACTIVE_SATELLITE table does not exist
    And the RAW_STAGE table contains data
      | CUSTOMER_ID | CUSTOMER_NAME | CUSTOMER_PHONE  | LOAD_DATE  | SOURCE |
      | 1001        | Alice         | 17-214-233-1214 | 1993-01-01 | *      |
  ...
  @fixture.multi_active_satellite
  Scenario: [SF-MAS-RM-04] Incremental load of a multi-active satellite with one value in rank column excluding NULL PKs, loads all records
    Given the MULTI_ACTIVE_SATELLITE table does not exist
    And the RAW_STAGE table contains data
      | CUSTOMER_ID | CUSTOMER_NAME | CUSTOMER_PHONE  | LOAD_DATE  | SOURCE |
      | 1001        | Alice         | 17-214-233-1214 | 1993-01-01 | *      |
  ...
  @fixture.multi_active_satellite
  Scenario: [SF-MAS-RM-05] Base load of a multi-active satellite with multiple and duplicated values in rank column loads first rank
    Given the MULTI_ACTIVE_SATELLITE table does not exist
    And the RAW_STAGE table contains data
      | CUSTOMER_ID | CUSTOMER_NAME | CUSTOMER_PHONE  | LOAD_DATE  | SOURCE |
      | 1001        | Alice         | 17-214-233-1214 | 1993-01-01 | *      |
  ...
  @fixture.multi_active_satellite
  Scenario: [SF-MAS-RM-06] Incremental load of a multi-active satellite with multiple and duplicated values in rank column loads all records
    Given the MULTI_ACTIVE_SATELLITE table does not exist
    And the RAW_STAGE table contains data
      | CUSTOMER_ID | CUSTOMER_NAME | CUSTOMER_PHONE  | LOAD_DATE  | SOURCE |
      | 1001        | Alice         | 17-214-233-1214 | 1993-01-01 | *      |
  ...
  @fixture.multi_active_satellite
  Scenario: [SF-MAS-RM-07] Base load of a multi-active satellite with one timestamp value in rank column loads all records
  ...
  @fixture.multi_active_satellite
  Scenario: [SF-MAS-RM-08] Incremental load of a multi-active satellite with multiple timestamps over different days in rank column loads all records
    Given the MULTI_ACTIVE_SATELLITE_TS table does not exist
    And the RAW_STAGE_TS table contains data
      | CUSTOMER_ID | CUSTOMER_NAME | CUSTOMER_PHONE  | LOAD_DATETIME           | SOURCE |
      | 1001        | Alice         | 17-214-233-1214 | 1993-01-01 11:14:54.396 | *      |
  ...
  @fixture.multi_active_satellite
  Scenario: [SF-MAS-RM-09] Base load of a multi-active satellite with multiple timestamps in the same day in rank column only loads first rank
    Given the MULTI_ACTIVE_SATELLITE_TS table does not exist
    And the RAW_STAGE_TS table contains data
      | CUSTOMER_ID | CUSTOMER_NAME | CUSTOMER_PHONE  | LOAD_DATETIME           | SOURCE |
