...
{%- if execute -%}
    {%- do exceptions.warn("Warning: This macro (multi_hash) is deprecated and will be removed in a future release. Use hash_columns instead.") -%}
{%- endif -%}
SELECT
{% for triple in triples -%}
    {%- if triple | length == 2 -%}

    {{ dbtvault.hash_deprecated(triple[0], triple[1]) }}

    {%- elif triple | length == 3 and triple | last == true -%}

    {{ dbtvault.hash_deprecated(triple[0], triple[1], triple[2]) }}
...
{%- macro hash_deprecated(columns, alias, sort=false) -%}
