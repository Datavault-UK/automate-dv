...
def test_sat_correctly_generates_sql_for_apply_source_filter_true(request, generate_model):
    ...
    dbt_result_first_run, _ = dbt_runner.run_dbt_models(dbt_init, mode='run',
                                                        model_names=[request.node.name],
                                                        full_refresh=True)

    dbt_result_inc_run, _ = dbt_runner.run_dbt_models(dbt_init, model_names=[request.node.name])
...
def test_sat_correctly_generates_sql_when_no_columns_added_all_excluded_incremental(request, generate_model):
    ...
    dbt_result, _ = dbt_runner.run_dbt_models(dbt_init, mode='run', model_names=[request.node.name])
