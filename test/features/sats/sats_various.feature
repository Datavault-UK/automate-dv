Feature: [SAT-PM-V] Satellite loads with period materialization using various time scales
  ...
  Scenario: [SAT-PM-V-01] Satellite load with microsecond interval will return an error.
    ...
    And the RAW_STAGE_TZ table contains data
      | CUSTOMER_ID | CUSTOMER_NAME | CUSTOMER_DOB | CUSTOMER_PHONE  | EFFECTIVE_FROM_TZ          | LOAD_DATE                  | SOURCE |
  ...
  Scenario: [SAT-PM-V-02] Satellite load with millisecond interval and intra-load duplicates.
    ...
    Then the SATELLITE_TZ table should contain expected data
      | CUSTOMER_PK | HASHDIFF                                               | CUSTOMER_NAME | CUSTOMER_DOB | CUSTOMER_PHONE  | EFFECTIVE_FROM_TZ          | LOAD_DATE                  | SOURCE |
