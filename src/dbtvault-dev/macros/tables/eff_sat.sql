...
{%- macro eff_sat(src_pk, src_dfk, src_sfk, src_ldts, src_eff_from, eff_start_date, eff_end_date, src_source, link, source)-%}

{%- set source_cols = dbtvault.get_src_col_list([src_pk, src_ldts, src_eff_from, eff_start_date, eff_end_date, src_source])-%}
...
, d as (SELECT
          ...
          ORDER BY {{ dbtvault.prefix([eff_end_date], 'c') }} ASC) = 1
...
, y AS (
  SELECT
    {{ dbtvault.prefix([src_pk, src_ldts, src_source, src_eff_from, eff_start_date, eff_end_date], 't') }}
    {% for dfk in src_dfk if not src_dfk is string %}
    ...
    ORDER BY {{ dbtvault.prefix([eff_end_date], 't') }} ASC) = 1
    THEN 'Y' ELSE 'N' END AS CURR_FLG
  FROM x
...
SELECT DISTINCT
  ...
  {{ dbtvault.prefix([src_eff_from], 'e') }} AS {{ eff_start_date }},
  TO_DATE({{ max_date }}) AS {{ eff_end_date }}
FROM {{ ref(source) }} AS e
{% if is_incremental() -%}
LEFT JOIN (
    SELECT {{ dbtvault.prefix(source_cols, 'd')}}
    FROM d
    WHERE d.CURR_FLG = 'Y' AND {{ dbtvault.prefix([eff_end_date], 'd') }}=TO_DATE({{ max_date }})
    ) AS eff
...
SELECT
  ...
  {{ dbtvault.prefix([src_source, src_eff_from, eff_start_date], 'y') }},
  CASE WHEN
  {% for dfk in src_dfk if not src_dfk is string %}
  ...
  y.DFK_1 IS NULL
  {% endfor %}
  THEN {{ dbtvault.prefix([src_eff_from], 'z') }} ELSE TO_DATE({{ max_date }}) END AS {{ eff_end_date }}
FROM y
LEFT JOIN {{ ref(source) }} AS z ON
{{ dbtvault.multikey(src_dfk, ['y', 'z'], 'join') }}
WHERE (y.CURR_FLG='Y' AND {{ dbtvault.prefix([eff_end_date], 'y') }}={{ max_date }})
{%- endif -%}
{% endmacro %}