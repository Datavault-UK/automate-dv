Feature: [SQLS-SAT-OOS-PM] Satellites Loaded using Period Materialization
  ...
  Scenario: [SQLS-SAT-OOS-PM-01] Base load of a satellite using full refresh should only contain first period records
  ...
  @fixture.satellite_cycle
  Scenario: [SQLS-SAT-OOS-PM-02] Satellite load over several daily cycles with insert_by_period into non-existent satellite
  ...
  @fixture.satellite_cycle
  Scenario: [SQLS-SAT-OOS-PM-03] Satellite load over several daily cycles with insert_by_period into empty satellite.
    Given the RAW_STAGE stage is empty
  ...
  @fixture.satellite_cycle
  Scenario: [SQLS-SAT-OOS-PM-04] Satellite load over several daily cycles with insert_by_period into populated satellite, with partial duplicates.
    Given the RAW_STAGE stage is empty
    And the SATELLITE sat is already populated with data
      | CUSTOMER_PK | HASHDIFF                             | CUSTOMER_NAME | CUSTOMER_DOB | EFFECTIVE_FROM | LOAD_DATE  | SOURCE |
      | md5('1001') | md5('1990-02-03\|\|1001\|\|ALBERT')  | Albert        | 1990-02-03   | 2019-05-04     | 2019-05-04 | *      |
  ...
  @fixture.satellite_cycle
  Scenario: [SQLS-SAT-OOS-PM-05] Satellite load over several daily cycles with insert_by_period into populated satellite, with all duplicates.
    Given the RAW_STAGE stage is empty
    And the SATELLITE sat is already populated with data
      | CUSTOMER_PK | HASHDIFF                             | CUSTOMER_NAME | CUSTOMER_DOB | EFFECTIVE_FROM | LOAD_DATE  | SOURCE |
      | md5('1001') | md5('1990-02-03\|\|1001\|\|ALBERT')  | Albert        | 1990-02-03   | 2019-05-04     | 2019-05-04 | *      |
  ...
  @fixture.satellite_cycle
  Scenario: [SQLS-SAT-OOS-PM-06] Satellite load over several daily cycles with insert_by_period into non-existent satellite, with date range.
    Given the SATELLITE table does not exist
    And the RAW_STAGE table contains data
      | CUSTOMER_ID | CUSTOMER_NAME | CUSTOMER_DOB | EFFECTIVE_FROM | LOAD_DATE  | SOURCE |
      | 1001        | Albert        | 1990-02-03   | 2019-05-04     | 2019-05-04 | *      |
  ...
  @fixture.satellite_cycle
  Scenario: [SQLS-SAT-OOS-PM-07] Satellite load over several daily cycles with insert_by_period into empty satellite, with date range.
    Given the RAW_STAGE stage is empty
    And the SATELLITE sat is empty
    When the RAW_STAGE is loaded
      | CUSTOMER_ID | CUSTOMER_NAME | CUSTOMER_DOB | EFFECTIVE_FROM | LOAD_DATE  | SOURCE |
  ...
  @fixture.satellite_cycle
  Scenario: [SQLS-SAT-OOS-PM-08] Satellite load over several daily cycles with insert_by_period into populated satellite, with partial duplicates and date range
    Given the RAW_STAGE stage is empty
    And the SATELLITE sat is already populated with data
      | CUSTOMER_PK | HASHDIFF                             | CUSTOMER_NAME | CUSTOMER_DOB | EFFECTIVE_FROM | LOAD_DATE  | SOURCE |
      | md5('1001') | md5('1990-02-03\|\|1001\|\|ALBERT')  | Albert        | 1990-02-03   | 2019-05-04     | 2019-05-04 | *      |
  ...
  @fixture.satellite_cycle
  Scenario: [SQLS-SAT-OOS-PM-09] Satellite load over several daily cycles with insert_by_period into populated satellite, with all duplicates and date range.
    Given the RAW_STAGE stage is empty
    And the SATELLITE sat is already populated with data
      | CUSTOMER_PK | HASHDIFF                             | CUSTOMER_NAME | CUSTOMER_DOB | EFFECTIVE_FROM | LOAD_DATE  | SOURCE |
      | md5('1001') | md5('1990-02-03\|\|1001\|\|ALBERT')  | Albert        | 1990-02-03   | 2019-05-04     | 2019-05-04 | *      |
